...
/**
 * @brief  Matrix 2x2 exponentiation.
 */
Matrix FiboByExp( Matrix A, uint64_t n )
{
    Matrix ans = {{{1, 0}, {0, 1}}}; // El calculo de nuestro resultado
    ...
    while(n > 0)
    {
         ...
         * resguarda en ans
         */        
        if(n%2 == 1) ans = product(ans, A);
        A = product(A, A); // Calculamos el cuadrado de A
        /*
